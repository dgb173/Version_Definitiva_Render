...
@app.route('/api/handicap_analysis/<string:match_id>')
def api_handicap_analysis(match_id):
    """
    Busca partidos anteriores de los equipos con un handicap similar.
    """
    handicap = request.args.get('handicap')
    is_neutral = request.args.get('neutral', 'false').lower() == 'true'
    if not handicap:
        return jsonify({'error': 'El parámetro de handicap es requerido.'}), 400

    try:
        # Llama a la nueva función del scraper
        datos = analizar_partidos_handicap(match_id, handicap, is_neutral)
        if "error" in datos:
            return jsonify(datos), 500
        return jsonify(datos)
    except Exception as e:
        logging.exception(f"Error en el análisis de handicap para el partido {match_id}")
